...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new Exception
...
object GoogleVsApple {
  ...
  def containsAny(list: List[String])(t: Tweet): Boolean = {
    list.exists(elem => t.text contains elem)
  }

  lazy val googleTweets: TweetSet = allTweets.filter(containsAny(google))
  lazy val appleTweets: TweetSet = allTweets.filter(containsAny(apple))
